...
public:
			...
			,primeIdx_(0)
	{
		...
		if (size != primes_[0])
		{
			...
			//用户传入的size值过大，已经超过了最后一个素数，调整的哦最后一个素数
...
public:
	...
	bool insert(int key)
	{
		...
		do 
		{
			if (table_[i].state_ != STATE_USEING)
			{
				table_[i].state_ = STATE_USEING;
				table_[i].key_ = key;
				useBucketNum_++;
				return true;
			}
		...
		//if (table_[idx].state_ != STATE_USEING)
		//{
		//	table_[idx].state_ = STATE_USEING;
		//	table_[idx].key_ = key;
		//	return true;
		//}
	...
	bool erase(int key)
	{
		...
		do
		{
			if (table_[i].state_ == STATE_USEING && table_[i].key_ == key)
			{
...
private:
	...
	void expand()
	{
		...
		{
			if (table_[i].state_ == STATE_USEING)//旧表有效的数据，重新放到扩容后的哈希表
			{
				int idx = table_[i].key_ % primes_[primeIdx_];
				int k = idx;
				do
				{
					if (newTable[k].state_ != STATE_USEING)
					{
						newTable[k].state_ = STATE_USEING;
